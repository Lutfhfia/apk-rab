...
### 2.1 Admin Keuangan
*   **Fungsi Utama:** Operasional harian anggaran, input RAB, realisasi pembayaran, dan koordinasi lapangan.
*   **Hak Akses:**
    *   Menginput pengajuan RAB baru melalui 4 skema pengeluaran dengan struktur dinamis.
    *   Mengelola draf pengajuan (hanya draf atau status ditolak yang dapat diedit/dihapus).
    *   Mengajukan draf/revisi RAB ke Manajer Keuangan (status menjadi `DIAJUKAN`).
    ...
    *   **Realisasi Pembayaran:** Melakukan pencairan pembayaran untuk RAB berstatus `DISETUJUI` dengan mengunggah bukti bayar transfer bank (Gambar/PDF, maks 5MB). Langkah ini **secara otomatis mencatat transaksi Dana Keluar (Kredit) pada Buku Arus Kas**.
...
### 2.2 Manajer Keuangan (Sistem Antrean Bersama / Pool System)
...
*   **Hak Akses:**
    ...
    *   **Monitoring Arus Kas:** Mengakses halaman Arus Kas (`/cash-flow`) dalam mode **Read-Only** (dapat memantau dan memfilter semua mutasi kas masuk/keluar, saldo berjalan, serta unduh bukti transfer, namun tidak dapat menginput transaksi baru).
...
[Admin Keuangan]
       ...
       └─► PILIHAN AKSI:
                 ...
                 └── [Ajukan RAB]    ──► Status: DIAJUKAN (Masuk antrean Manajer, data terkunci)
                           │
...
Setelah disetujui Manajer Keuangan, status RAB berubah menjadi `DISETUJUI_MANAJER` dan masuk ke antrean Direktur untuk mendapatkan keputusan akhir (`DISETUJUI`). Jika disetujui Direktur, status berubah menjadi `DISETUJUI` dan siap dicairkan oleh Admin Keuangan.
...
Admin Keuangan klik "Upload Bukti Bayar"
       ...
       ├─► Unggah Bukti Transfer Bank (Format Gambar/PDF, Maks 5MB)
       │
       ▼
...
Admin dan Manajer Keuangan dapat menghasilkan dokumen laporan fisik instan yang bersih, rapi, dan terformat secara profesional:
...
### 4.6 Panel Catatan Diskusi Kontekstual (NEW)
Setiap modal detail RAB dilengkapi dengan panel **Catatan Diskusi** interaktif yang berfungsi sebagai ruang obrolan kontekstual antara Admin Keuangan, Manajer Keuangan, dan Direktur:
...
### 4.7 Penstabil Gulir Halaman Arus Kas - Scroll Position Maintenance (NEW)
...
### 4.8 Pratinjau File Bukti Terintegrasi (Modal File Viewer) (NEW)
...
*   **Deteksi Cerdas:** Skrip Javascript mendeteksi ekstensi berkas secara cerdas. File berekstensi PDF akan dimuat di dalam bingkai iframe web terintegrasi, sedangkan file berformat gambar (PNG, JPG, JPEG, WEBP) akan dirender secara dinamis di dalam tag penampil gambar responsif.
...
| :--- | :--- | :--- | :--- | :--- |
| **-** | Simpan Draft | `DRAFT` | Admin Keuangan | Data disimpan lokal, bisa diedit/dihapus |
| **DRAFT** / **DITOLAK** | Ajukan RAB | `DIAJUKAN` | Admin Keuangan | Terkunci bagi Admin, masuk antrean Manajer. Menyimpan status sukses diajukan dalam session untuk pemicu modal sukses & notifikasi WA |
| **DIAJUKAN** | Tolak (Revisi) | `DITOLAK` | Manajer Keuangan | Kembali terbuka untuk diedit/dihapus Admin, wajib mengisi catatan alasan revisi |
| **DIAJUKAN** | Setujui Tahap 1 | `DISETUJUI_MANAJER` | Manajer Keuangan | Masuk antrean persetujuan akhir Direktur |
| **DISETUJUI_MANAJER** | Tolak (Revisi) | `DITOLAK` | Direktur | Kembali terbuka untuk diedit/dihapus Admin, wajib mengisi catatan alasan revisi |
| **DISETUJUI_MANAJER** | Setujui Akhir | `DISETUJUI` | Direktur | Terkunci secara final, siap dicairkan/dibayarkan Admin |
| **DISETUJUI** | Upload Bukti Bayar | `SELESAI` | Admin Keuangan | Terkunci permanen, otomatis tercatat di Arus Kas sebagai `DANA_KELUAR` (Kredit), mengurangi total saldo berjalan |
...
*   **Keuntungan:** Sistem Antrean Bersama meningkatkan throughput operasional di PT SBK secara dramatis. Siapa pun Manajer Keuangan yang aktif dapat segera memproses antrean. 
...
| :--- | :--- | :--- | :--- | :--- |
| 1 | **Admin Keuangan** | `luthfiandini1909@gmail.com` | `andin2005` | Input RAB, WhatsApp Share, Upload Bukti Transfer |
| 2 | **Manajer Keuangan** | `manajer@rab-sbk.com` | `password` | Review, Persetujuan Tahap 1, Buku Kas Masuk, Ekspor PDF |
...
4.  **Menyimpan Dokumen:**
    ...
    *   Klik tombol **Ajukan RAB** jika data sudah final dan ingin langsung dikirim ke Manajer Keuangan.
5.  **Pengiriman WhatsApp (Penting):**
    ...
    *   Kirimkan pesan tersebut ke Manajer Keuangan. Anda juga bisa mengklik tombol logo WhatsApp di baris tabel RAB berstatus Diajukan kapan saja untuk mengirim ulang pesan.

##### Langkah 3: Berkolaborasi via Catatan Diskusi
...
##### Langkah 4: Realisasi Pembayaran (Upload Bukti Transfer)
...
2.  Isi data bukti pembayaran:
    ...
    *   Klik **Pilih File Bukti Transfer** dan unggah foto/PDF bukti bayar (maksimal 5MB).
3.  Klik **Simpan Pembayaran**.
4.  Status RAB otomatis berubah menjadi `Selesai`, dan sistem secara otomatis mencatatkan pengeluaran transfer ini sebagai **Dana Keluar** pada Buku Arus Kas global.

---

#### B. OPERASIONAL MANAJER KEUANGAN (Review, Kas Masuk, & Ekspor PDF)

##### Langkah 1: Login dan Memantau Antrean
1.  Login menggunakan Email: `manajer@rab-sbk.com` dan Password: `password`.
2.  Anda akan diarahkan ke **Dashboard Manajer Keuangan**.
...
5.  Klik tombol **Lihat Bukti** pada baris tabel untuk meninjau pratinjau bukti bayar secara instan di dalam popup modal (PDF termuat dalam iframe, Gambar termuat dalam peninjau foto).
...
3.  Klik **Ekspor PDF** untuk mengunduh rekapitulasi keuangan bulanan dengan format profesional.
...
1.  Pilih menu **Arus Kas** pada sidebar kiri.
2.  Di halaman ini, Anda dapat memantau saldo total kas, melakukan pencarian mutasi kas masuk/keluar, menyaring rentang tanggal, serta mengklik tombol **Lihat Bukti** untuk memeriksa file pendukung mutasi kas secara instan tanpa bisa memanipulasi atau menambahkan data kas baru.
