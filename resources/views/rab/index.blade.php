                    ...
                    <td class="py-4 px-5">
                        <div class="flex items-center space-x-1">
                            ...
                            @if($rab->status === \App\Enums\RabStatus::DRAFT)
                                ...
                                @if($rab->updated_at->diffInDays(now()) >= 30)
                                <form method="POST" action="{{ route('rab.destroy', $rab) }}" class="inline" onsubmit="return confirm('Yakin ingin menghapus RAB ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition" title="Hapus (draft > 30 hari)">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                </form>
                                @endif

                            {{-- DIAJUKAN: Detail, WhatsApp --}}
...
@if($rab->status === \App\Enums\RabStatus::DRAFT || $rab->status === \App\Enums\RabStatus::DITOLAK)
