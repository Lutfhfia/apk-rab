{{-- Modal Pencairan / Transfer Dana RAB --}}
<div id="paymentModal-{{ $rab->id }}" class="fixed inset-0 bg-black/50 z-[60] hidden items-center justify-center p-4">
    <div class="bg-gray-50 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col overflow-hidden animate-fade-in">
        <div class="p-5 bg-white border-b border-gray-100 flex items-center justify-between shrink-0">
            <div>
                <h3 class="text-lg font-bold text-gray-800">Pencairan & Transfer Dana</h3>
        ...
        <div class="p-6 overflow-y-auto grow">
            ...
            <form method="POST" action="{{ route('rab.payment.store', $rab) }}" enctype="multipart/form-data" id="paymentForm-{{ $rab->id }}">
                @csrf
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
                    <h3 class="text-base font-bold text-gray-800 mb-5">Data Pencairan / Transfer</h3>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1.5">Tanggal Transfer *</label>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1.5">Nominal Transfer *</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1.5">Metode Transfer *</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1.5">Rekening Tujuan Admin</label>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1.5">Nama Penerima Admin</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1.5">Bukti Transfer * (JPG, PNG, PDF max 1MB)</label>
                ...
                <div class="flex items-center justify-end space-x-3">
                    ...
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl text-sm font-bold transition">Konfirmasi Cairkan Dana</button>
                </div>
            </form>
        </div>
    </div>
...
(function() {
    ...
    function validateFileSize(input) {
        ...
        if (file.size > maxSize) {
            alert('Ukuran file bukti transfer maksimal adalah 1MB. File yang Anda pilih berukuran: ' + (file.size / (1024*1024)).toFixed(2) + 'MB.');
