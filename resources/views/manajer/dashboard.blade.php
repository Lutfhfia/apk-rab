...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="relative bg-gradient-to-r from-[#1E293B] via-[#334155] to-[#0F172A] rounded-2xl p-8 overflow-hidden shadow-lg">
        <div class="absolute top-0 right-0 w-72 h-72 bg-blue-500/10 rounded-full -translate-y-1/2 translate-x-1/4 blur-3xl"></div>
        <div class="relative z-10 flex items-center justify-between">
            <div>
                <p class="text-blue-400 text-sm font-semibold mb-1">👋 Selamat Datang,</p>
                <h2 class="text-white text-2xl font-extrabold">{{ Auth::user()->name }}</h2>
                <p class="text-slate-400 text-sm mt-1">Monitoring & approval Rancangan Anggaran Biaya</p>
            </div>
    ...
    <div class="bg-white/50 backdrop-blur-sm p-4 rounded-xl flex flex-wrap items-end justify-between gap-4 border border-gray-100">
        <form method="GET" action="{{ route('manajer.dashboard') }}" class="flex flex-wrap items-end gap-4 w-full md:w-auto">
            <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-6">
        <div class="p-5 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-base font-bold text-gray-800">RAB Menunggu Persetujuan Anda</h2>
            <span class="bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full">{{ $roleWaiting }} menunggu</span>
        </div>
            ...
            <table class="sticky-table w-full text-left text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                        ...
                        <td class="py-4 px-5">
                            <a href="{{ route('manajer.rab.index', ['status' => $rab->status->value, 'open_rab_id' => $rab->id]) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition">Review</a>
                        </td>
                    </tr>
                    @empty
